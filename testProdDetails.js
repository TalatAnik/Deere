...
// const outputFile = "output/output_test_01.json"
...
const prodMissFile = 'output/missed/missedProds_test.json'

const inpDataB64 = process.argv.find((a) => a.startsWith('--input-data')).replace('--input-data', '')
const inputData = JSON.parse(Buffer.from(inpDataB64, 'base64').toString())

// console.log("inputData: ",JSON.stringify(inputData.url, null, 2))
...
async function main (urlArg, outputFile, cacheFolder) {
...
async function runner(url, outFile, cFolder)
{
  try {
    await main(url, outFile, cFolder)
  } catch (error) {
...
void runner(inputData.url, inputData.outputFile, inputData.cache)